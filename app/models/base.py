...
class User(Base):
    ...
    epic_password = Column(String(256), nullable=True, comment='加密存储的Epic密码（旧方式，已弃用）')
    epic_device_id = Column(String(128), nullable=True, comment='Epic Device Auth ID（旧方式，已弃用）')
    epic_device_secret = Column(String(256), nullable=True, comment='加密存储的Epic Device Auth Secret（旧方式，已弃用）')
    epic_refresh_token = Column(String(512), nullable=True, comment='加密存储的Epic Refresh Token')
...
class PushLog(Base):
    ...
    game_name = Column(String(128), nullable=False, index=True, comment='游戏名称')
    game_slug = Column(String(128), nullable=True, comment='游戏标识（用于关联Redis数据）')
    push_time = Column(DateTime, server_default=func.now(), comment='推送时间')
    status = Column(Boolean, default=False, comment='推送状态，0=失败，1=成功')
    is_next_week = Column(Boolean, default=False, comment='是否为下周预告推送')
