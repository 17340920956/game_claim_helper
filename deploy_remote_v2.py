...
def run_command(client, cmd, description="", timeout=600):
    """执行远程命令"""
    if description:
        print(f"\n>>> {description}")
    print(f"执行: {cmd[:80]}...")
...
def main():
    ...
    try:
        ...
        run_command(
            client,
            f"echo '{PASSWORD}' | sudo -S bash -c 'cd {REMOTE_PATH} && tar xzf /tmp/game_claim_helper.tar.gz --overwrite 2>/dev/null || true'",
            "解压代码"
        )
        ...
        run_command(
            client,
            f"echo '{PASSWORD}' | sudo -S docker stop game_claim_helper 2>/dev/null || true",
            "停止容器"
        )
        run_command(
            client,
            f"echo '{PASSWORD}' | sudo -S docker rm game_claim_helper 2>/dev/null || true",
            "删除容器"
        )
        ...
        run_command(
            client,
            f"echo '{PASSWORD}' | sudo -S docker image prune -f 2>/dev/null || true",
            "清理镜像"
        )
