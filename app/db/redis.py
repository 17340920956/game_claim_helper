...
class RedisClient:
    ...
    def clear_week_data(self, is_next_week: bool = False) -> bool:
        ...
        notified_users_pattern = f"notified_users:{prefix}:*"
        for key in self.client.scan_iter(match=notified_users_pattern):
            self.client.delete(key)
        
        user_status_pattern = f"user_status:{prefix}:*"
        for key in self.client.scan_iter(match=user_status_pattern):
            self.client.delete(key)
